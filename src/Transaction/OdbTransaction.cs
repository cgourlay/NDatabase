...
namespace NDatabase.Transaction
{
    ...
    internal sealed class OdbTransaction : ITransaction
    {
        ...
        {
            Log4NetHelper.Instance.LogInfoMessage(string.Format("OdbTransaction: Adding WriteAction in Transaction of session {0}", _session.GetId()));
                ...
                _numberOfWriteActions > StorageEngineConstant.MaxNumberOfWriteObjectPerTransaction)
            {
                ...
                Log4NetHelper.Instance.LogInfoMessage("OdbTransaction: Number of objects has exceeded the max number " + numberOfWriteActions + "/" +
        ...
        private void CommitMetaModel()
        {
            ...
            Log4NetHelper.Instance.LogDebugMessage("OdbTransaction: Start commitMetaModel");
            ...
            while (enumerator.MoveNext())
            {
                ...
                Log4NetHelper.Instance.LogDebugMessage(string.Format("OdbTransaction: Analysing class {0}", newClassInfo.FullClassName));
                Log4NetHelper.Instance.LogDebugMessage(string.Format("\t-Commited CI   = {0}", newClassInfo));
                ...
                Log4NetHelper.Instance.LogDebugMessage(string.Concat("\t-Commiting new Number of objects = ",
        ...
        private void ApplyTo()
        {
            if (!_isCommited)
            {
                Log4NetHelper.Instance.LogInfoMessage("OdbTransaction: can not execute a transaction that is not confirmed");
                return;
            }
