...
namespace NDatabase.Core.BTree
{
    ...
    internal sealed class LazyOdbBtreePersister : IBTreePersister, ICommitListener
    {
        ...
        public IBTreeNode LoadNodeById(object id)
        {
            ...
            try
            {
                Log4NetHelper.Debug(string.Format("LazyOdbBtreePersister: Loading node with id {0}", oid));
        ...
        public void SaveNode(IBTreeNode node)
        {
            ...
            if (node.GetId() == StorageEngineConstant.NullObjectId)
            {
                try
                {
                    ...
                    Log4NetHelper.Debug(string.Format("LazyOdbBtreePersister: Saved node id {0}", oid));
        ...
        public IBTree LoadBTree(object id)
        {
            ...
            try
            {
                Log4NetHelper.Debug(string.Format("LazyOdbBtreePersister: Loading btree with id {0}", oid));
        ...
        public void SaveBTree(IBTree treeToSave)
        {
            try
            {
                ...
                if (oid == null)
                {
                    ...
                    Log4NetHelper.Debug(string.Format("LazyOdbBtreePersister: Saved btree {0} with id {1} and  root {2}", treeToSave.GetId(), oid,
                                                    treeToSave.GetRoot()));
        ...
        private void Persist()
        {
            ...
            Log4NetHelper.Debug(string.Concat("LazyOdbBtreePersister: ", "persist ", _nbPersist.ToString(), "  : Saving " + count + " objects - ",
            ...
            while (iterator.MoveNext())
            {
                ...
                Log4NetHelper.Debug(string.Concat("LazyOdbBtreePersister: ", "Committing oid " + oid, " | ", i.ToString(), "/", size.ToString(),
                                            " | ", (t1 - t0).ToString(), " ms"));

                i++;
            }

            Log4NetHelper.Debug(string.Concat("LazyOdbBtreePersister: ", nbCommited.ToString(), " commits / ", size.ToString()));
        }
