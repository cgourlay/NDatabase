...
namespace NDatabase.Core.BTree
{
    ...
    internal sealed class LazyOdbBtreePersister : IBTreePersister, ICommitListener
    {
        ...
        public IBTreeNode LoadNodeById(object id)
        {
            ...
            try
            {
                Log4NetHelper.Instance.LogDebugMessage(string.Format("LazyOdbBtreePersister: Loading node with id {0}", oid));
        ...
        public IBTree LoadBTree(object id)
        {
            ...
            try
            {
                Log4NetHelper.Instance.LogDebugMessage(string.Format("LazyOdbBtreePersister: Loading btree with id {0}", oid));
        ...
        public void SaveBTree(IBTree treeToSave)
        {
            try
            {
                ...
                if (oid == null)
                {
                    ...
                    Log4NetHelper.Instance.LogDebugMessage(string.Format("LazyOdbBtreePersister: Saved btree {0} with id {1} and  root {2}", treeToSave.GetId(), oid,
                                                    treeToSave.GetRoot()));
        ...
        private void Persist()
        {
            ...
            Log4NetHelper.Instance.LogDebugMessage(string.Concat("LazyOdbBtreePersister: ", "persist ", _nbPersist.ToString(), "  : Saving " + count + " objects - ",
            ...
            while (iterator.MoveNext())
            {
                ...
                Log4NetHelper.Instance.LogDebugMessage(string.Concat("LazyOdbBtreePersister: ", "Committing oid " + oid, " | ", i.ToString(), "/", size.ToString(),
                                            " | ", (t1 - t0).ToString(), " ms"));

                i++;
            }

            Log4NetHelper.Instance.LogDebugMessage(string.Concat("LazyOdbBtreePersister: ", nbCommited.ToString(), " commits / ", size.ToString()));
        }
